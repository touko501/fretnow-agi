...
// ═══ POST /admin/production-reset — DISABLED IN PRODUCTION ═══
// SECURITY: Cet endpoint est désactivé. Utiliser psql directement pour un reset.
// Pour réactiver en dev uniquement, décommenter le bloc ci-dessous.
router.post('/production-reset', authenticate, isAdmin, async (req, res) => {
  if (process.env.NODE_ENV === 'production') {
    return res.status(403).json({
      error: 'Endpoint désactivé en production. Utiliser psql directement.',
      documentation: 'Voir ARCHITECTURE.md section "Maintenance"',
    });
  }
  return res.status(403).json({ error: 'Endpoint désactivé. Contactez l\'admin système.' });
});
